...
export function ArticleManager() {
  ...
  return (
    <section className="bg-white/50 dark:bg-neutral-900/50 border border-neutral-200/60 dark:border-neutral-800/60 rounded-3xl p-6 md:p-8 backdrop-blur-xl shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-0.5">Gerencie seus artigos publicados e arquivados.</p>
          </div>
        </div>
      ...
      {error && (
        <div className="mb-6 px-4 py-3 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 text-sm text-red-600 dark:text-red-400 flex items-center gap-2 animate-in slide-in-from-top-1">
        ...
        <div className="flex flex-col gap-3">
          {articles.map((article) => {
            ...
            return (
              <div
                key={article._id}
                className="group flex items-center justify-between gap-4 px-4 py-3 rounded-xl border border-neutral-200/80 dark:border-neutral-700/50 bg-white dark:bg-neutral-900/80 shadow-sm transition-all hover:border-purple-200 dark:hover:border-purple-800/50"
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <Link
                    to={`${ADMIN_PATH}/articles/${article.slug}/edit`}
                    title="Editar"
                    className="p-2 rounded-lg text-neutral-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-500/10 transition-colors"
                  >
          ...
          {articles.length === 0 && (
            <div className="w-full flex flex-col items-center justify-center text-center text-neutral-400 py-16">
              ...
              <p className="text-sm mt-1 opacity-70 max-w-[250px]">Comece criando seu primeiro artigo.</p>
            </div>
          )}
        </div>
      </div>
